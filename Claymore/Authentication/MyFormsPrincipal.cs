...
namespace Claymore.Authentication
{
    /// <summary>
   ...
   where TUserData : class, new()
    {
        ...
        public static void SignIn(string loginName, TUserData userData, int expiration)
        {
            ...
            if (userData != null)
            {
                data = Helper.MyJsonHelper.SerializeObject(userData); //XmlHelper.XmlSerialize(userData, System.Text.Encoding.UTF8);
            }
            //如果有必要,可以在这里再对date进行一次加密
        ...
        public static MyFormsPrincipal<TUserData> TryParsePrincipal(HttpRequest request)
        {
            ...
            try
            {
                ...
                {
                    // 3. 还原用户数据
                    userData = Helper.MyJsonHelper.DeserializeObject<TUserData>(ticket.UserData);// XmlHelper.XmlDeserialize<TUserData>(ticket.UserData, System.Text.Encoding.UTF8);
                }
